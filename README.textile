...
* Added ability to use ImageLoader for local images, just use the Uri path of the file "#106":https://github.com/novoda/ImageLoader/issue/106
...
Add the following code to initialise and provide access to the ImageLoader. The settings builder gives you some control over the caching and network connections.
...
When you want to load an image into an ImageView, you just get the ImageLoader from the Application class and call the load method.
